...
REM 全量验证（前端类型检查+构建、go vet、go test、gofmt 漂移检查）
verify.bat
...
[`docs/SYSTEM_REMEDIATION_PLAN.md`](docs/SYSTEM_REMEDIATION_PLAN.md) §3「目标架构」的分层图，概要如下：
...
根目录 `verify.bat` 是发布门禁，依次执行（任一失败即非零退出）：

1. **前端类型检查 + 生产构建**：`npm run build`（即 `vue-tsc -b && vite build`，在 `frontend/` 下）。
2. **后端静态检查**：`go vet ./...`。
3. **后端测试**：`go test -count=1 ./...`。
4. **格式漂移检查**：`gofmt -l backend` 输出必须为空。
...
前端 `package.json` 目前只有 `dev` / `build` / `preview`；前端自动化测试与 lint 尚在计划内（见
`docs/FRONTEND_SIMPLIFY_PLAN.md` 与整改计划 P3），尚未纳入 `verify.bat`。
...
`release.bat` 见 **Phase 6 交付**（与本文档同批落地，详见
[`docs/RELEASE_RUNBOOK.md`](docs/RELEASE_RUNBOOK.md)）。产物与升级/回滚步骤以该手册为准，要点：
...
完整错误码表见 [`docs/ERROR_CODES.md`](docs/ERROR_CODES.md)。错误响应统一为
...
- **升级/回滚演练**：见 [`docs/RELEASE_RUNBOOK.md`](docs/RELEASE_RUNBOOK.md)。
...
| [`docs/SYSTEM_REMEDIATION_PLAN.md`](docs/SYSTEM_REMEDIATION_PLAN.md) | 整改主计划（当前总纲，含问题台账 P0-P3 与 Phase 0-6 状态） |
| [`docs/MEMORA_FUNCTION_SPEC.md`](docs/MEMORA_FUNCTION_SPEC.md) | 基于源码的功能说明（数据模型、路由、模块职责） |
| [`docs/FRONTEND_SIMPLIFY_PLAN.md`](docs/FRONTEND_SIMPLIFY_PLAN.md) | 前端「小白化」简化改版计划 |
| [`docs/SECRET_SCAN_REPORT.md`](docs/SECRET_SCAN_REPORT.md) | 密钥与敏感文件泄露扫描记录 |
| [`docs/RELEASE_RUNBOOK.md`](docs/RELEASE_RUNBOOK.md) | 发布手册（tag 驱动发布、升级、回滚、冒烟清单） |
| [`docs/ERROR_CODES.md`](docs/ERROR_CODES.md) | 错误码表（code / HTTP 状态 / 含义 / 用户建议） |
